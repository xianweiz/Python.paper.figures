...
pl.xticks(tick_locs+bar_width/2., IDs,  rotation=rotation_angle, fontsize=20)
#locs = [9,18,27,36]
#newIDs=['com','spec','parsec','bio']
#pl.xticks(locs, newIDs,  rotation=0, fontsize=16)

ax = pl.gca()
ind = 0
for text_header in cfg_headers_list:
	text_loc = NUM_SCHEMES*(ind+0.5)
	pl.text(text_loc, tick_high+0.03, cfg_headers_list[ind], fontsize=24)
	line_loc = NUM_SCHEMES*(ind+1)+0.5
	ax.axvline(line_loc, linestyle='-.', color='k', linewidth=1.0) # vertical lines
	ind += 1
