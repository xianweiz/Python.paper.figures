...
graph('(np.exp(-(np.log(x) - mu)**2 / (2 * sigma**2)) / (x * sigma * np.sqrt(2 * np.pi)))',
	   	pl.linspace(1e-2, 1e3, 10000), 'black')
...
#graph('0.5+0.5*math.erf( (np.log(x)-mu)/np.sqrt(2*sigma**2) )',
#	   	pl.linspace(1e-2, 1e3, 10000), 'red')

cdf_x = [0.032, 0.064, 0.128, 0.256, 0.512, 1.024, 5, 10, 64, 100, 500, 1000]
print cdf_x
cdf_y = [lognorm_cdf(xx, mu, sigma) for xx in cdf_x]
print cdf_y
pl.plot(cdf_x,cdf_y, color='black', linestyle='--', marker='x', markersize=10)

pl.text(0.1e2,1e-3, 'pdf', fontsize=40)
pl.text(0.6e1,1e-1, 'cdf', fontsize=40)
